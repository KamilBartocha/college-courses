lag.plot1=function(data1,max.lag=1,corr=TRUE,smooth=FALSE){ 
  ...
  for(h in 1:max.lag){                       
   plot(lag(data1,-h), data1, xy.labels=F, main=paste(name1,h,")",sep=""), ylab=name2, xlab="") 
    if (smooth==TRUE) 
    lines(lowess(ts.intersect(lag(data1,-h),data1)[,1],
                 ts.intersect(lag(data1,-h),data1)[,2]), col="red")
    if (corr==TRUE)
    legend("topright", legend=round(a[h], digits=2), text.col ="blue", bg="white")
   }
}
...
lag.plot2=function(data1,data2,max.lag=0,corr=TRUE,smooth=FALSE){ 
  ...
  for(h in 0:max.lag){                   
   plot(lag(data1,-h), data2, xy.labels=F, main=paste(name1,h,")",sep=""), ylab=name2, xlab="") 
    ...
    if (corr==TRUE)
    legend("topright", legend=round(a[m1-h], digits=2), text.col ="blue", bg="white")             
   }
}
...
sarima.for=function(xdata,nahead,p,d,q,P=0,D=0,Q=0,S=-1,tol=sqrt(.Machine$double.eps)){ 
  data=as.ts(xdata) 
  n=length(data)
  constant=1:n
  xmean=matrix(1,n,1)
  if (d==0 & D==0) {
    fitit=arima(data, order=c(p,d,q), seasonal=list(order=c(P,D,Q), period=S),
            xreg=xmean,include.mean=F, optim.control=list(reltol=tol));
    nureg=matrix(1,nahead,1)        
} else if (xor(d==1, D==1)) {
    fitit=arima(data, order=c(p,d,q), seasonal=list(order=c(P,D,Q), period=S),
            xreg=constant,include.mean=F, optim.control=list(reltol=tol));
    nureg=(n+1):(n+nahead)       
} else { fitit=arima(data, order=c(p,d,q), seasonal=list(order=c(P,D,Q), period=S), 
